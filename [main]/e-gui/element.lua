...
addEventHandler('onClientRender', root, function()
    for _, element in pairs(elements) do
        if element.type == 'editbox' and getKeyState('mouse1') and element.focused then
        ...
        elseif element.type == 'editbox' and state == 'down' and element.active then
            if x >= element.x and x <= element.x + element.w and y >= element.y and y <= element.y + element.h then
                ...
                -- else
                    ...
                    animate(element.test,element.w, 'Linear', 300, function (s)
                        element.test = s
                    end)
                ...
                element.focused = false
                animate(element.test,0, 'Linear', 300, function (s)
                    element.test = s
                end)
            end
