...
function createGPS(road)
	...
	for i, node in ipairs(road) do
		if i == 1 then
			GPS.road[i] = {}
			GPS.road[i].marker = createColSphere(node.x, node.y, getGroundPosition(node.x, node.y, node.z + 0.3), 10)
				...
				if localPlayer == plr and dim then
					...
					for i,row in ipairs(GPS.road) do
						if row.ID >= roadNum then
							if #GPS.road > 0 then
								exports["e-notifications"]:sendNotification("success", "Dojechałeś na miejsce")
							end
							removeGPS()
						end
					end
...
function getPath(startNode, endNode)
	...
	while true do
		...
		if not bestNode then
			if getElementInterior(localPlayer) == 0 and getElementDimension(localPlayer) == 0 then
				exports["e-notifications"]:sendNotification("error", "Nie można odnależć trasy")
			end
