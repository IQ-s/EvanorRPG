local a = loadstring([[local __internal = {
    __internal = {}
}
...
local function renderLineChart(data,chart)
    ...
    if data.blur then
        exports['pl-blur']:dxDrawBlur(x, y, w, h, data.mask, 255, true)
    end
    ...
    if isMouseInPosition(x, y, w, h) then
        ...
        for k,v in pairs(defaultChartData) do
            if isMouseInPosition(x + v.x*w - pointSize/2, y + v.y*h - pointSize/2, pointSize, pointSize) then
                dxDrawText((data.tipFormat or '%d'):format(min + (max - min)*(1-v.y)), x + v.x*w, y + v.y*h - tipMargin, nil, nil, tocolor(255,255,255,a[tostring(chart) .. '-alpha']), 1, font, 'center', 'center', false, false, false, true)
            end
        end
    end
end
